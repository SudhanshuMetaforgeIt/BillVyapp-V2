...
import type { SupportStatusSlice } from '../types/support.types';

type SupportStatusDonutProps = {
  total: number;
  slices: SupportStatusSlice[];
};
...
export function SupportStatusDonut({
  total,
  slices,
}: SupportStatusDonutProps) {
  let offset = 0;
    ...
    <div className="flex flex-col items-center gap-5 sm:flex-row sm:items-center sm:gap-6">
      <div className="relative size-36 shrink-0">
        <svg viewBox="0 0 140 140" className="size-full -rotate-90" role="img">
          <title>Support ticket status distribution</title>
          <circle
            cx="70"
            cy="70"
            r={RADIUS}
